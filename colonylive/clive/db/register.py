...
def write_csv_table(fname, exps):
    ...
    w.writerow(['ExpID', 'Project', 'PlateID', 'Conditions',
                'Datetime', 'ScannerID', 'Position'])
    ...
    for exp in exps:
        ...
        out = [exp.id, exp.project, exp.plate_id, exp.conditions,
               dt_str, sid, pos]
        w.writerow(out)
...
def report_by_email(person_id, fname):
    ...
    send_mail.send_mail('Colony-live', email,
                        'Registration information', body, fname)
"""
...
if __name__ == "__main__":
    argvs = sys.argv
    if len(argvs) != 9:
        raise ValueError("usage: %s [person_id] [project] [dt_start] [plate_ids] [medium] [h_scan] [conditions_key] [conditions_value]" % argvs[0])
    values = argvs[1:]
